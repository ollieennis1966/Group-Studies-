...
def initial_cloud(diameter = 1e-3):
    ...
    for i in range(0, N):
        ...
        XY.append(sqrt(x**2 + y**2))

    #print("Temperature = ", Temp_0*1e6, "microK")
    #print("radius = ", radius*1e3, "mm")
    #print("mean speed = ", format(np.mean(Speed)*1e3, '.2f'), "mm s-1")
...
def fall_expansion(dt = 1e-4, g = 9.81, n = 20):
    ...
    for j in range(0, n): # Motion of cloud before 1st pulse broken into "n" steps
        for i in range(0, N): # loop over all atoms
            ...
            #vx, vy, vz = Speed[i]*sin(theta_v)*cos(phi_v), Speed[i]*sin(theta_v)*sin(phi_v), Speed[i]*cos(theta_v) # Velocity components
            #velocity[i] = Vector(vx, vy, vz) # Velocity vector         
            
        mean_speed = np.mean(Speed) # Mean atom speed after time dt
...
def interval(pulse, T = 1e-2, g = 9.81, n = 20):
    ...
    """
    Speed_1, Distance_1, Speed_2, Distance_2 = [], [], [], []

    du = Vector(1e-3, 1e-3, 1e-3)
    ...
    for j in range(0, n): # Motion of cloud between pulses broken into "n" steps
        for i in range(0, int(0.5*N)): # Atoms excited by 1st π/2 pulse
            ...
            vx, vy, vz = Speed_1[i]*sin(theta_v)*cos(phi_v), Speed_1[i]*sin(theta_v)*sin(phi_v), Speed_1[i]*cos(theta_v) # Velocity components
            velocity[i] = Vector(vx, vy, vz) # Velocity vector  
        ...
        for i in range(int(0.5*N) + 1, N): # Atoms excited by 1st π/2 pulse
            ...
            vx, vy, vz = Speed_2[m]*sin(theta_v)*cos(phi_v), Speed_2[m]*sin(theta_v)*sin(phi_v), Speed_2[m]*cos(theta_v) # Velocity components
            velocity[i] = Vector(vx, vy, vz) # Velocity vector  
